...
SERVICESツールでは、発電所・ポンプ場・ごみ処理施設・消防署・警察署・病院・学校・公園を道路から16m以内のクリックで道路脇に配置できます。それぞれ専用の敷地・建物Definition・地形標高を持ち、道路や既存敷地・塗られた区画とは重ねられません。簡易Meshは種類ごとに形を変え、公園は屋外敷地として描画します。施設の供給は道路ネットワーク上の到達距離と容量で判定します。電力・水道は現状、接続された道路網全体へ供給する暫定モデルで、個別の送電線・水道管を結ぶ操作はまだありません。人口・就業者から建物別の簡易需要を集計し、右側のCITY SERVICESパネルとDebugに供給率・需要・容量を表示します。
...
- SERVICES: 8種の施設建物から選択し、道路から16m以内をクリックして道路脇の空き敷地に設置。CITY SERVICESのMANAGE FACILITIESで撤去します。設置費用は即時、維持費は600ゲーム秒ごとに計上します。設置・撤去はUndo/Redoできます
